...
**Cambio:** Agregar `average_rating` y `total_ratings` al tipo `CourseDetail`.
...
export interface CourseDetail extends Course {
  description: string;
  classes: Class[];
  // Hereda average_rating y total_ratings de Course
}
```

**Nota:** Ya existe en `Course`, solo verificar que se propague correctamente.
...
```tsx
// Agregar despues de .stats y antes de .classesSection:
{/* Rating Section */}
{course.id && (
  <RatingSection
    courseId={course.id}
...
**Cambio:** Agregar estilos para la seccion de ratings.
...
### 6.4 `src/app/course/[slug]/page.tsx`

**Cambio:** Verificar que la API retorne `average_rating` y `total_ratings` (ya deberia funcionar si el Backend los incluye en `/courses/{slug}`).

**Revision necesaria:** Confirmar que `getCourseData` recibe estos campos del Backend.
